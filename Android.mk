...
LOCAL_SRC_FILES :=	CPP/myWindows/myGetTickCount.cpp \
			...
			C/7zCrc.c \
			C/7zCrcOpt.c
LOCAL_CFLAGS := -O2 -pipe -s -D_FILE_OFFSET_BITS=64 -D_LARGEFILE_SOURCE -DNDEBUG -D_REENTRANT -DENV_UNIX -D_7ZIP_LARGE_PAGES -DBREAK_HANDLER -DUNICODE -D_UNICODE -frtti -fexceptions
LOCAL_C_INCLUDES := $(LOCAL_PATH) $(LOCAL_PATH)/C $(LOCAL_PATH)/CPP $(LOCAL_PATH)/CPP/myWindows $(LOCAL_PATH)/CPP/include_windows
ifeq ($(TARGET_ARCH),arm)
    ifeq ($(ARCH_ARM_HAVE_ARMV7A),true)
        LOCAL_LDFLAGS += $(LOCAL_PATH)/prebuilt/arm-linux-androideabi/armeabi-v7a/libgnustl_shared.so
    else
        LOCAL_LDFLAGS += $(LOCAL_PATH)/prebuilt/arm-linux-androideabi/armeabi/libgnustl_shared.so
    endif
endif
SYMLINKS := $(addprefix $(TARGET_OUT_OPTIONAL_EXECUTABLES)/,7z)
$(SYMLINKS): 7ZA_BINARY := $(LOCAL_MODULE)
$(SYMLINKS): $(LOCAL_INSTALLED_MODULE)
	@echo "Symlink: $@ -> $(7ZA_BINARY)"
	@mkdir -p $(dir $@)
	@rm -rf $@
	$(hide) ln -sf $(7ZA_BINARY) $@

ALL_DEFAULT_INSTALLED_MODULES += $(SYMLINKS)
include $(BUILD_EXECUTABLE)

ifeq ($(TARGET_ARCH),arm)
    # p7zip depends on -fexceptions which requires '__cxa_end_cleanup' symbol and others
    # that aren't provided by the default toolchain.
    # Copy libgnustl_shared.so from android-ndk-r9c and install it to /system/lib to workaround this issue.
    include $(CLEAR_VARS)
    ...
    ifeq ($(ARCH_ARM_HAVE_ARMV7A),true)
        LOCAL_SRC_FILES := prebuilt/arm-linux-androideabi/armeabi-v7a/libgnustl_shared.so
    else
        LOCAL_SRC_FILES := prebuilt/arm-linux-androideabi/armeabi/libgnustl_shared.so
    endif
